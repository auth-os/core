...
library PayableApp {
  ...
  function pay0() public pure returns (bytes memory) {
    return abi.encodeWithSelector(PAYS, uint(0));
  }

  // forwards payment to one address
  function pay1(address _dest, uint _amt) public pure returns (bytes memory) {
    return abi.encodeWithSelector(PAYS, uint(1), _amt, _dest);
  }

  // forwards payment to 2 addresses
  function pay2(
    address _dest1, uint _amt1, address _dest2, uint _amt2
