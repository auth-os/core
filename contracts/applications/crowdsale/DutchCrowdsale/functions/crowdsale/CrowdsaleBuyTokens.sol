...
library CrowdsaleBuyTokens {
  ...
  */
  function buy(bytes _context) public view returns (bytes32[] store_data) {
    ...
    uint current_rate;
    uint spend_amount;
    uint start_rate = uint(read_values[5]);
    uint end_rate = uint(read_values[6]);
    uint start_time = uint(read_values[7]);
    ...
    if (
        ...
        uint(read_values[7]) + uint(read_values[8]) <= now // Crowdsale has already ended
    ) {
      triggerException(ERR_INSUFFICIENT_PERMISSIONS);
    }
    // Sanity checks - starting and ending rates, and team wallet should be nonzero
    assert(start_rate != 0 && end_rate != 0 && read_values[9] != bytes32(0));

    // Crowdsale is allowing purchases - get current sale rate -
    current_rate = start_time - now;
    // Sanity check - time elapsed should be less than than the duration (should have thrown earlier)
    assert(current_rate <= uint(read_values[8]));
    // Add decimals for precision
    current_rate = 100 * current_rate;
    current_rate = current_rate / uint(read_values[8]);
    current_rate *= (start_rate - end_rate);
    current_rate /= 100;
    current_rate = start_rate - current_rate;
    // Sanity check
    assert(start_rate >= current_rate);

    // If wei_sent * current_rate is more than the number of tokens remaining:
    if (wei_sent * current_rate > tokens_remaining) {
      // Get amount of wei able to be spent
      spend_amount =
        (tokens_remaining - (tokens_remaining % current_rate)) / current_rate;
    ...
    require((current_rate * spend_amount) + uint(read_values[2]) > uint(read_values[2]));
    stPush(ptr, bytes32(current_rate * spend_amount + uint(read_values[2])));
    // Subtract purchased token amount from tokens_remaining
    stPush(ptr, TOKENS_REMAINING);
    stPush(ptr, bytes32(tokens_remaining - (spend_amount * current_rate)));
