...
// ImplementationConsole with additional testing features
contract TestImplementationConsole {
  ...
  function TestImplementationConsole(address _storage) public {
  ...
  */
  function addFunctions(bytes32 _app, bytes32 _version, bytes4[] _function_sigs, address[] _function_addrs, bytes _context) public
  ...
  @return store_data: A formatted storage request - first 64 bytes designate a forwarding address (and amount) for any wei sent
  */
  function describeFunction(bytes32 _app, bytes32 _version, bytes4 _function_sig, bytes _function_description, bytes _context) public
  returns (bytes32[] store_data) {
    // Ensure input is correctly formatted
    require(_context.length == 96);
    require(_app != bytes32(0) && _version != bytes32(0));
